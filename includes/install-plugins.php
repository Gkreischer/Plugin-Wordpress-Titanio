...
add_action('wp_ajax_instalar_plugin', 'instalar_plugin');
add_action('wp_ajax_nopriv_instalar_plugin', 'instalar_plugin');
...
function instalar_plugin()
{
    ...
    if ($tmp_file_handle) {
        ...
        if (is_wp_error($install_result)) {
            ...
            $installed_plugin_file = trailingslashit(WP_PLUGIN_DIR) . $plugin_slug;

            // Ative o plugin
            $activation_result = activate_plugin($installed_plugin_file);
            if (is_wp_error($activation_result)) {
                wp_send_json_error('Erro ao ativar o plugin: ' . $activation_result->get_error_message() . ' caminho: ' . $installed_plugin_file);
            } else {
